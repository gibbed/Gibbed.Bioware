...
{
    [OpcodeHandler(Script.Opcode.CPDOWNSP)]
    public class CopyDownStackPointer : IInstruction
    {
        public int Size
        {
            get { return 8; }
        ...
        {
            this.Destination = BitConverter.ToInt32(code, offset + 2).BigEndian();
            this.ByteCount = BitConverter.ToInt16(code, offset + 6).BigEndian();
            offset += 8;
        }

        public void Print(IPrintState state)
        {
            state.Print("cpdownsp", this.Destination, this.ByteCount);
        }
    }
}
