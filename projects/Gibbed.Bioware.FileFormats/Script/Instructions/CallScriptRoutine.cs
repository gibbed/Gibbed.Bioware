...
{
    [OpcodeHandler(Script.Opcode.JSR)]
    public class CallScriptRoutine : IInstruction
    {
        public int Size
        {
            get { return 6; }
        ...
        public void Encode(byte[] code, ref int offset, out OperandType operand, IState state)
        {
            ...
            Array.Copy(BitConverter.GetBytes(state.IndexToOffset(this.Destination).LittleEndian()), 0, code, offset + 2, 4);
            offset += 6;
        }
        ...
        {
            state.Print("call", state.GetLabel(this.Destination));
        }
    }
}
